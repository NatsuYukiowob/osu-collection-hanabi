/* Farm helper (刷圖助手) page — see netlify/functions/farm-helper.js for
   ...
   the 太難了/太簡單 feedback store. Ported from catch-tracker's own
   render-farm-helper.js, itself modeled closely on mania-tracker.com's own
   farm-helper:
   - Landing (no ?id=): search/recent/explainer + a peer pp ladder,
...
const FARM_HELPER_RECENT_KEY = 'st_farm_helper_recent';
...
function renderLanding(main) {
    ...
    const loggedInUser = getStLoggedInUser();
    main.innerHTML = `
...
// its pp relative to the top of this list. A plain scrollable list, own row
// auto-scrolled into view once rendered — reuses the site's own existing
// ranked-bar visual language (see .farm-helper-target-bar-fill /
// .stat-bar-fill) instead of a bespoke widget.
...
async function loadFarmHelper() {
    ...
    if (!userId) {
        document.title = `Std Tracker — ${t('farm_helper_title')}`;
        renderLanding(main);
        return;
    }
    ...
    try {
        ...
        document.title = `Std Tracker — ${t('farm_helper_title')}: ${p.username || userId}`;
