/* Community-submitted Discord server directory — ported from catch-tracker's
   own communities.js (itself mania-tracker.com's "Discord 服務器" tab: a
   crowd-sourced list anyone can add to, no moderation queue), scoped to
   std here. Simplified filters: just a search box + free-text tags, no
   region/multi-tag filter grid. Same signed-token write-auth pattern as
   goals.js — no new auth mechanism.

   No Cache-Control on the GET response: this list is re-fetched right
   after the viewer's own submit/remove, and a cached response would serve
   the browser its own stale pre-mutation copy — a real bug caught (and
   fixed) on catch-tracker's own version of this file. */
