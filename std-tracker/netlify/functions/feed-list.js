/* Public, filtered/paginated view over the live score feed
   ...
   _scores-poll-core.js). Ported from catch-tracker's own feed-list.js
   (itself modeled on the main site's farm-maps-list.js): pure read-only
   GET, no auth, in-module TTL cache over the gzip'd dataset, and an honest
   `coverage` block read fresh from scores-poll-state every call so the
   frontend can show real freshness instead of implying the feed is
   exhaustive. */
const { getFeedStore, getRankingsStore } = require('./_blobs-store');
const { getJSONGz } = require('./_blob-json');
const { VALID_GRADES } = require('./_std-constants');

const PAGE_SIZE = 30;
const DS_CACHE_TTL_MS = 20_000; // shorter than a slow-moving dataset's cache — this is meant to feel live
