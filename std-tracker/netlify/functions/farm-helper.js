/* Farm helper (刷圖助手): recommends specific maps by estimated pp gain,
   ...
   (mirrors mania-tracker.com's farm helper, and catch-tracker's own port
   of it) rather than any difficulty/pp estimation — no rosu-pp-js or
   similar needed.
   ...
   PP-gain formula: weightedPpSum(topPpDesc) + bonusPp. bonusPp is constant
   per player and cancels out in a before/after delta, so the gain from
   inserting one candidate score is just the weighted-sum delta with no
   need to know the player's real total pp at all.
...
const PEER_WINDOW_EACH_SIDE = 50; // ~100 peers total, per catch-tracker's own approved plan
...
exports.handler = async (event) => {
    ...
    try {
        ...
        const [rankings, maps, token, prefs] = await Promise.all([
            ...
            getJSONGz(mapsStore, 'maps:global').then(m => m || []),
            getOsuToken(),
            fetchPrefs(userId),
        ]);
