/* Single-player profile: combines the cached rankings:global record (rank/pp
   maintained by our own crawler) with a LIVE call to GET /users/{id}/osu for
   fields we don't crawl ourselves (join_date, play_time, career
   grade_counts, bio, monthly playcounts) and a LIVE call to
   GET /users/{id}/scores/best for "best plays" — these live calls aren't
   cached; a single profile-page view is cheap enough that a second cache/
   de-dup layer isn't worth it for v1. If the player isn't in the cached
   rankings (e.g. queried by id directly, not yet swept), the live call also
   backfills the rank/pp/accuracy fields instead of failing outright.
   mostUsedMod is derived here from bestPlays so the client doesn't need to
   re-implement the tally.

   Ported from catch-tracker's own player-get.js, including its rank-delta
   block, now that _rank-snapshot-core.js's daily snapshot exists here too. */
...
exports.handler = async (event) => {
    ...
    try {
        ...
        if (liveUser) {
            ...
            // only started polling scores ourselves from this site's own
            // launch and have no historical per-day data of our own to show.
            ...
            // The bulk rankings endpoint (no country filter) comes back with
            // country_rank always null on every entry — only the per-user
            // endpoint reliably has it, so backfill from here rather than
            // leaving the cached rankings sweep's null in place.
        ...
        // a fixed "N days ago", since this grows day by day until it hits
        // RETENTION_DAYS. Positive global/country = moved UP (rank number
        // went down); positive pp = gained pp. Silently omitted if we have
        // no snapshot yet or this player wasn't in it.
        try {
