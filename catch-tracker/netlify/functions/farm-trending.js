/* "全球刺分熱門" (mania-tracker's own global farm/most-played map leaderboard,
   NOT the personalized per-player Farm Helper at farm-helper.js) — which
   maps the tracked TW pool has actually been grinding lately. Derived
   entirely from feed:recent (the same dataset feed-list.js already reads),
   grouped by beatmap_id, rather than a new crawler/dataset: feed:recent is
   a real recent-activity window (FEED_CAP-capped, see _catch-constants.js),
   so "most scores in it" already IS a genuine live "trending now" signal —
   arguably a better one than a lifetime play_count total (maps-list.js's
   own playcount_desc sort), which never resets and always favours old
   maps. */
