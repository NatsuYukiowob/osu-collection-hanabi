/* Daily rank-history snapshot, powering the player page's rank-delta
   arrows (▲/▼ next to the global/country/pp badges). Unlike the other
   crawlers here this makes NO osu! API calls of its own — it just reads
   the already-fresh `rankings:global` dataset (kept current by
   ...
   compact per-day blob. player-get.js later reads the OLDEST snapshot
   still within RETENTION_DAYS and diffs it against a player's current
   live numbers.
   ...
   only started 2026-09-12, deltas will be against whatever the oldest
   available day is (a few days old at first, growing toward
   RETENTION_DAYS) rather than a fixed "90 days ago" — see
   player-get.js's rankDelta() for how that's surfaced honestly. */
