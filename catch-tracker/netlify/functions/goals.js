/* Personal goal tracking — mania-tracker.com's own 目標 page, scoped down
   to just its "個人 > 總PP" goal type for a first pass (see the plan this
   shipped from: user picked the cheapest-to-verify type first, the other 7
   — 大分/成績數量/排名/單圖通過/FC/準確率/評級 — need more per-type osu! API
   calls and were deliberately left for later rather than half-built now).

   Auth: same signed-token pattern as farm-helper-prefs.js — reads are
   public-by-user-id (harmless, no sensitive data), writes require the
   token proving "this request really is that osu! user", minted at login
   (osu-replay-callback.js / _auth-token.js). No new auth mechanism. */
