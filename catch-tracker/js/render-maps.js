...
// main site's own batch-download feature already uses (js/osu.js there),
// ?no_video=true keeps the file small like that feature does too.
...
function mapCard(set) {
    ...
    // Forward-only backfill (same pattern as grade_counts/rank_delta
    // elsewhere on this site) — a set the crawler hasn't re-touched since
...
function makeRangeSlider({ prefix, max, gap, formatLabel }) {
    ...
    return {
        ...
        // sent at all — same convention the star slider already used.
